...
public String autenticar(@RequestParam String email,
    ...
    // se já estiver logado, não deixa logar de novo
    if (session.getAttribute("emailUsuario") != null) {
        return "redirect:/paginabko";
    }
    ...
    // valida usuário, status e senha usando BCrypt
    if (usuario != null
            && Boolean.TRUE.equals(usuario.getStatus())
            && passwordEncoder.matches(senha, usuario.getSenha())) {

        // autenticação OK → grava dados mínimos na sessão
        session.setAttribute("emailUsuario", email);
        session.setAttribute("grupoUsuario", usuario.getGrupo().name());

        // redireciona para a MESMA página que você já usa
        return "redirect:/paginabko";
    }

    // erro de login (credenciais inválidas ou usuário inativo)
